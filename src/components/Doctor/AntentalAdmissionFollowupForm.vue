      ...
      <base-card class="mt-2">
        ...
        <form action="#" @submit.prevent="validateData">
          <div class="row ps-5">
            ...
            ></textarea>
            <p class="error-hint" v-if="!instructions.isValid">You can't submit empty form. Please write some instructions</p>
          </div>
  ...
  methods: {
    ...
    async submitForm(){
      ...
      const id=this.$route.params.patientId;
         const antenatalAdmissionFollowUp={
          patient_id: id,
          user_id: localStorage.getItem("userId"),
          follow_up_and_instructions: this.instructions.value,
       }

        console.log(antenatalAdmissionFollowUp);
         try{
         await this.$store.dispatch('doctor/addData',{
          value:antenatalAdmissionFollowUp,
          path:"http://localhost:8000/api/post_vice_doctor_antenatal_admission_follow_up_sheet",
          mutation:"pushAntenatalAdmissionFollowUp",
        });
        this.$router.replace(`/doctor/${id}/AntentalAdmissionFollowup`);
        }catch(error){
